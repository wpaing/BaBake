...
export const generateInvoicePDF = (client: Client, order: Order, measurement?: Measurement, currency?: Currency) => {
  ...
  doc.autoTable({
    ...
    body: tableBody,
    theme: 'grid',
    headStyles: { fillColor: PRIMARY_PURPLE, fontSize: 10, halign: 'left' },
    columnStyles: {
...
export const generateFinancialReport = (transactions: Transaction[], period: string, currency?: Currency) => {
  const doc = new jspdf.jsPDF();
  const sym = currency?.symbol || 'MMK';
